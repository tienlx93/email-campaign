...
- `DELETE /campaigns/:id` — Delete a **draft** campaign (auth required)
- `POST /campaigns/:id/schedule` — Schedule a campaign (set `scheduled_at`); allow user to cancel by set to null (auth required)
- `POST /campaigns/:id/send` — Simulate sending (mark recipients as `sent`, record `sent_at`, set campaign status `sent`) (auth required)
- `GET /campaigns/:id/stats` — Return rates and counts (auth required)
